...
Note: You *have* to use an old version of MongoDB due to some of these old libraries' database server APIs. MongoDB 3 is known to work ok.
...
The form is completely functional. It works by receiving the profile information from the `req.body` and passing it, as-is to the template. This, however, means, that the attacker is able to control a variable that flows directly from the request into the view template library.
